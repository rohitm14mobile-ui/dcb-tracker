  ...
  const processedData = useMemo(() => {
    ...
    sorted.forEach((txn) => {
      ...
      if (category.isRent) {
        ...
        feeNote = `+ ₹${fee.toLocaleString("en-IN", {
      ...
      // --- NEW SPLIT REWARDS MATH START ---
      const rawBaseRP = category.baseEligible
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    value={
                      newTxn.cardHolder ||
                      (cardholders.length > 0 ? cardholders[0] : "")
                    }
                    onChange={(e) =>
              ...
              {isEditing && (
                <div className="bg-amber-50 p-4 rounded-xl border border-amber-200 space-y-2 mt-2">
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (
                          !refundInput ||
                          isNaN(refundInput) ||
                          Number(refundInput) <= 0
                        )
                          return;
                        const newAmt = Math.max(
                          0,
                          Number(newTxn.amount) - Number(refundInput)
                        );
                  ...
                  <p className="text-[10px] text-amber-700 font-medium leading-tight">
                    This automatically subtracts the refund from the main amount
                    above and adds a note.
                  </p>
                </div>
              )}
